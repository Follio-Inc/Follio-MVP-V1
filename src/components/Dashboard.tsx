...
const Dashboard: React.FC<DashboardProps> = ({ user, parsedData, onLogout, onLoginRequest }) => {
    ...
    <div className="flex items-center gap-3">
      ...
      {user ? (
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-full bg-blue-600 px-6 py-2 text-sm font-semibold text-white transition hover:bg-blue-700 shadow-sm shadow-blue-500/20"
        >
          Publish to Follio
          ...
          <div className="grid gap-6">
            <div className="rounded-[2rem] border border-slate-200/60 bg-white/80 p-8 shadow-lg shadow-blue-500/10 backdrop-blur-md transition hover:border-blue-200/50 hover:shadow-blue-500/15">
              ...
              <div className="mt-8 grid gap-5 sm:grid-cols-3">
                ...
                <div className="group rounded-3xl border border-sky-100 bg-sky-50/50 p-5 text-sky-700 transition hover:bg-sky-50">
                  <div className="text-4xl font-bold tracking-tighter text-sky-800">{profileData.education.length}</div>
                  <p className="mt-2 text-xs font-bold uppercase tracking-wide opacity-80">Education</p>
                </div>
          ...
          <div className="space-y-10">
            <div className="rounded-[2.5rem] border border-slate-200/60 bg-white/80 p-10 shadow-xl shadow-blue-500/10 backdrop-blur-md">
              <div className="flex items-center justify-between">
                ...
                <span className="rounded-full bg-sky-50 px-4 py-1.5 text-xs font-bold uppercase tracking-wide text-sky-700 ring-1 ring-sky-100">
                  {profileData.education.length} schools
                </span>
              </div>
              <div className="mt-8 space-y-5">
                {profileData.education.map((edu) => (
                  <div key={edu.id} className="group rounded-3xl border border-sky-100/60 bg-sky-50/30 p-6 transition hover:border-sky-200 hover:bg-sky-50/60">
                    <p className="text-xs font-bold uppercase tracking-wide text-sky-700">{edu.school}</p>
